...
class TaskPolicy(UtgBasedInputPolicy):
    ...
    def __init__(self, device, app, random_input, workflow_json):
        ...
        try:
            ...
            self._chat = genai.GenerativeModel("gemini-1.5-flash").start_chat()
        except Exception as e:
    ...
    def generate_event_based_on_utg(self):
        ...
        if current_state.get_app_activity_depth(self.app) < 0:
            ...
            if current_task is None:
                stop_app_intent = self.app.get_stop_intent()
                self.logger.info("Workflow Completed. Stopping app...")
                return IntentEvent(intent=stop_app_intent)
        
            action, candidate_actions = self._get_action_with_LLM(current_state)
            if action is not None:
                self.__action_history.append(current_state.get_action_desc(action))
                if self._is_task_complete(current_state):
                    self.task_history.append(self.current_task_id)
                    self.current_task_id = self._get_next_task_id()
                    self.__action_history = []
    ...
    def _get_current_task(self):
        for node in self.workflow["nodes"]:
            if node["id"] == self.current_task_id:
                return node
        return None
    
    def _get_next_task_id(self):
        for edge in self.workflow["edges"]:
            if edge["source"] == self.current_task_id:
                if edge["target"] not in self.task_history:
                    return edge["target"]
        return None
    
    def _is_task_complete(self, current_state):
        task_description = self.current_task['data']['label']
        prompt = f"Given the task '{task_description}' and the current state description:\n{current_state.state_str}\nIs the task complete? Answer with 'yes' or 'no'."
        response = self._query_llm(prompt)  # Pass current state description
        if response is None:
            return False
        return "yes" in response.lower()
        
    def _query_llm(self, full_prompt, model_name="gemini-1.5-flash"):
        if self._chat is None:
            self.logger.error("Cannot query LLM. Gemini chat session not intialized.")
            return None
    ...
    def _get_action_with_LLM(self, current_state):
        state_prompt, candidate_actions = current_state.get_described_actions()
        if not candidate_actions:
            self.logger.warning("No candidate actions found in the current state.")
            return None, None

        history_string = "\n".join([f"- {action}" for action in self.__action_history]) if self.__action_history else "No actions taken yet."
        
        print("Current Task", self.current_task)
        print("CAndidate actions", candidate_actions)

        full_prompt = (f"Task: {self.current_task['data']['label'] if self.current_task else 'N/A'}\n"
                        f"Completed Steps:\n{history_string}\n"
                        f"Current State:\n{state_prompt}\n"
                        f"Which action ID should I choose next (0-{len(candidate_actions) - 1})? If no action is needed, return -1.  "
                        f"If the selected action requires text input, provide the input text after the action ID, separated by a colon. "
                        f"For example: '2:Enter this text'"
                        f"NOTE: The app is already started and ready to go, you can ignore the start task and report `yes` to the start task complete questions")
        
        print(full_prompt)
        
        response = self._query_llm(full_prompt)
        if response is None:
            self.logger.error("LLM query failed. Returning None.")
            return None, None
        ...
        try:
            ...
            if 0 <= idx < len(candidate_actions):
                ...
                self.logger.info("LLM indicated no action needed.")
                return None, None
            else:
                self.logger.error(f"Invalid action ID from LLM: {idx}.  Response: {response}")
                return None, None  # Handle invalid IDs more gracefully.
        except ValueError:
            self.logger.error(f"Could not parse action ID from LLM response: {response}")
            return None, None
